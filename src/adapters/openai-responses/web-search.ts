 ...
 * attached Codex's default `web_search` declaration. The direct Meta host below is the one
 * destination whose non-Contributor default no such list contains, so it is admitted by
 * destination instead.
 */
 ...
 * shape, and Luna remains untouched. Two predicates admit the gate, and either one is
 * sufficient on its own: a Contributor id names one gateway family wherever it is reached
 * from, so a relay of that gateway is covered without trusting the host; and the direct
 * Meta destination is the whole predicate there, because Meta's refusal is a gateway schema
 * rule for every Muse model it serves, including its non-Contributor default. A malformed,
 * credentialed, or parameterized destination still keeps its original body when the id
 * alone does not admit it. Keep the rejected names together so a newly identified field is
...
export function stripMuseSparkUnsupportedWebSearchFields(
  ...
  if (!isPlainObject(body)) return body;
  const strictModel = typeof modelId === "string"
  ...
  if (!strictModel) {
    ...
    if (destination !== MUSE_SPARK_STRICT_ANY_MODEL_DESTINATION) return body;
  }
