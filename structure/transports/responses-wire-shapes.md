...
preserving preview tools. Two predicates admit that gate and either is sufficient: a Contributor
Muse id names one gateway family wherever it is reached from, so a relay of the same gateway is
covered without trusting the host; and on the direct Meta destination
(`https://api.meta.ai/v1/responses`) the destination is the whole predicate, because Meta's
refusal is a gateway schema rule for every Muse model it serves, its default `muse-spark-1.3` is
not a Contributor id, and a missing model id still strips. A destination that is neither that host
nor carrying a Contributor id keeps its body.
